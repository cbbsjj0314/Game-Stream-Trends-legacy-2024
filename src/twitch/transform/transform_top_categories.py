# src/twitch/transform/transform_top_categories.py
    ...
    while start_time <= end_time:
        ...
        try:
            ...
            for raw_json in json_list:
                for idx, entry in enumerate(raw_json.get("data", [])):
                    standardized_data.append(
                        {
                            "id": str(entry.get("id", None)),
                            "name": str(entry.get("name", None)),
                            "igdb_id": str(entry.get("igdb_id", None)),
                            "rank": idx + 1,
                        }
                    )

            df = pd.DataFrame(standardized_data).astype(
                {"id": "string", "name": "string", "igdb_id": "string", "rank": "Int64"}
            )

            collected_at_value = max(timestamps) if timestamps else f"{formatted_date}_00-00-00"
